...
public class ProgParser {
	...
	public static ic.ast.Node Parse(fun.parser.Tree parseTree) {
	...
	private static ic.ast.Node constructAst(fun.parser.Tree parseTree) {
	...
	private static ic.ast.Node constructBlock(fun.parser.Tree parseTree,
		...
		List<Statement> statements = new ArrayList<Statement>();
		if (subtrees.length == 0) {
			return new StmtBlock(((Token) parseTree.root).line, statements);
		}
		while (true) {
